...
export default function ContentEditor() {
  ...
  const loadFile = async (filePath: string, branch = 'main') => {
    try {
      const response = await fetch(`/api/cms/content?path=${encodeURIComponent(filePath)}&branch=${branch}`);
      const data = await response.json();
      ...
      <div className="w-1/4 bg-white shadow-lg p-4">
        ...
        {status && (
          <div className="mb-4 p-3 bg-gray-50 rounded">
            ...
            <p className="text-xs text-gray-600">Current Branch: {status.status?.currentBranch}</p>
