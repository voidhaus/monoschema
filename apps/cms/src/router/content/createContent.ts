import { createContentInputSchema, createContentOutputSchema, CreateContentInput, CreateContentOutput, Content } from "@voidhaus/cms-types";
...
const createContentResolver = resolver<
  CreateContentInput,
  CreateContentOutput
>(async (input, context) => {
  ...
  for (const property of blockType.properties) {
    let propertyType: StringConstructor | NumberConstructor | BooleanConstructor | DateConstructor;
  ...
  if (input.properties) {
    ...
    if (!validationResult.valid) {
      const errorMessages = validationResult.errors.map((error: { path: string; message: string }) => 
        `${error.path}: ${error.message}`
      ).join(', ');
      throw new Error(`Content properties validation failed: ${errorMessages}`);
    }
  }
  ...
  const result = await data.Client.insertOne<Content>(
    "content",
    contentData
  );
