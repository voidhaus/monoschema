 ...
 */
export function createRpcApp<T>(definition: T, config: RpcConfig): RpcApp<T> {
  return {
...
const callProcedureLocal = async <T>(
  request: JsonRpcRequest,
  context: RpcContext,
  definition: T,
  config: RpcConfig,
  ...
  try {
    ...
    if (!execution.success) {
      return createErrorResponse(-32603, execution.error, jsonRpcRequest.id);
    }

    // Apply output validation if enabled
    const finalResult = await applyOutputValidation(
      execution.result,
 ...
 */
async function applyOutputValidation(
  result: unknown,
